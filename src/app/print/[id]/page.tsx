...
export default function PrintPatientPage({ params }: { params: { id: string } }) {
  const [patient, setPatient] = useState<PatientType | null>(null);
  const [payments, setPayments] = useState<PaymentType[]>([]);

  useEffect(() => {
    const fetch = async () => {
      const res1 = await axios.get(`/api/patients/${params.id}`);
      const res2 = await axios.get(`/api/payments?patientId=${params.id}`);
