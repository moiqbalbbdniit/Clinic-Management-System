...
export default function PrintByMonthPage() {
  ...
  const handleFetch = async () => {
    ...
    try {
      ...
      if (res.data.length === 0) {
        toast.info(`No patients found for ${dayjs().month(Number(month) - 1).format("MMMM")} ${year}.`);
      } else {
        toast.success(`${res.data.length} patients found for ${dayjs().month(Number(month) - 1).format("MMMM")} ${year}.`);
      }
  ...
  const handleDownloadPDF = () => {
    ...
    doc_landscape.text(`Obonti Piles Clinic`, doc_landscape.internal.pageSize.width / 2, 15, { align: 'center' });
    doc_landscape.setFontSize(12);
    doc_landscape.setTextColor(0, 0, 0); // Black
    doc_landscape.text(`Dr. Abhijit Kumar Vishwas`, doc_landscape.internal.pageSize.width / 2, 22, { align: 'center' });
    doc_landscape.setFontSize(10);
    doc_landscape.text(`Monthly Patient Report - ${dayjs().month(Number(month) - 1).format("MMMM")} ${year}`, doc_landscape.internal.pageSize.width / 2, 29, { align: 'center' });
    ...
    autoTable(doc_landscape, {
      ...
      body: patients.map((p, index) => {
        ...
        return [
          ...
          p.address,
          p.disease,
          p.totalCost.toFixed(0), // Format Total Cost
          totalPaid.toFixed(0),   // Format Paid
          balance.toFixed(0)      // Format Balance
        ];
      }),
      styles: {
      ...
      didDrawPage: function (data) {
        ...
        doc_landscape.text(`Page ${data.pageNumber} of ${pageCount}`, doc_landscape.internal.pageSize.width - 20, doc_landscape.internal.pageSize.height - 10, { align: 'right' });
        doc_landscape.text(`Generated on ${new Date().toLocaleDateString('en-IN')}`, 14, doc_landscape.internal.pageSize.height - 10, { align: 'left' });
        doc_landscape.text(`This receipt is system generated. Thank you for trusting Obonti Piles Clinic.`, doc_landscape.internal.pageSize.width / 2, doc_landscape.internal.pageSize.height - 5, { align: 'center' });
      }
    });
  ...
  return (
    <main className="max-w-5xl mx-auto px-4 py-10 space-y-6">
      ...
      <div className="flex flex-wrap gap-4 justify-center items-end">
        <div>
          <label htmlFor="month-select" className="block mb-1 text-sm">Select Month</label>
          <select
            id="month-select"
            value={month}
          ...
          >
            ...
            {Array.from({ length: 12 }, (_, i) => i + 1).map((m) => (
              <option key={m} value={String(m).padStart(2, "0")}>{dayjs().month(m - 1).format("MMMM")}</option>
            ))}
          </select>
        </div>
        <div>
          <label htmlFor="year-input" className="block mb-1 text-sm">Select Year</label>
          <input
            id="year-input"
            type="number"
            value={year}
        ...
        <Button onClick={handleFetch} className="h-10">Fetch Data</Button>
      </div>
      ...
      {readyToPrint && patients.length > 0 && (
        <div className="text-center">
          <p className="mb-4">{patients.length} patients found for {dayjs().month(Number(month) - 1).format("MMMM")} ${year}.</p>
      ...
      {readyToPrint && patients.length === 0 && (
        <div className="text-center text-gray-600">
          ...
          <p>Please adjust your selection or ensure patients exist for that period.</p>
        </div>
      )}
    </main>
  );
